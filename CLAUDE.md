# CLAUDE.md

This file provides guidance to Claude Code (claude.ai/code) when working with code in this repository.

## Repository Overview

Personal dotfiles for macOS / WSL2 / Linux + zsh, and **native Windows PowerShell**, managed via symlinks.

- **macOS / WSL2 / Linux**: `install.sh` creates symlinks (zsh + `.sh` hooks)
- **Windows (native PowerShell)**: `install.ps1` creates symlinks (PowerShell profile + `.ps1` hooks)

**Fresh-machine onboarding**: `bash ~/dotfiles/install.sh` (or `pwsh -File install.ps1`) → edit `~/.gitconfig.local` で identity を入れる → 初回 commit が通る。`~/.gitconfig.local` 未記入のままだと commit が "Please tell me who you are" で fail-loud。

## Installing / Applying Changes

### Windows (native PowerShell)

```powershell
# Developer Mode を有効にするか、管理者として実行
pwsh -File ~/dotfiles/install.ps1          # winget インストール + symlink 作成（冪等）
. $PROFILE                                 # プロファイルをリロード
winget import --import-file ~/dotfiles/winget-packages.json --ignore-versions  # パッケージのみ再実行
```

### macOS / WSL2 / Linux (zsh)

```bash
bash ~/dotfiles/install.sh        # Full setup: OS 検出・依存インストール・symlink 作成（冪等）
source ~/.zshrc                   # Reload shell config after .zshrc edits
```

`install.sh` を再実行すると symlink 部分は "skip: symlink already correct" が出るだけで安全（冪等性は `link()` ヘルパーが `readlink` 比較で担保）。

## Verifying changes

変更後は以下を順に流す（CI / ローカル両用）:

```bash
zsh -n ~/dotfiles/.zshrc                                    # zsh 構文チェック
bash -n ~/dotfiles/install.sh ~/dotfiles/test/bootstrap-check.sh   # bash 構文チェック
DOTFILES=~/dotfiles bash ~/dotfiles/test/bootstrap-check.sh # 必須ファイル存在 + .zshrc 構文の最小検証
docker build -f Dockerfile.test -t dotfiles-test . && docker run --rm dotfiles-test  # コンテナで install.sh をフル実行
```

`Dockerfile.test`: `install.sh` を非対話 Linux コンテナでフル実行する CI シミュレータ。lint や単体テストは持たず、bootstrap が冪等で通ることを担保する。

shell ファイル編集時の lint は PostToolUse hook で自動的に走る。手動で再現したい場合は `shellcheck` (`.sh` / `.zsh`) と `PSScriptAnalyzer` (`.ps1`) が必要 — `brew install shellcheck` / `winget install Microsoft.PowerShell` 経由で入れる。

PowerShell 側 (`install.ps1` / プロファイル) は手元の Windows 環境で `pwsh -File install.ps1` を再実行して "skip" / "ok" 出力のみを確認する。

## Symlink Map

### Windows (native PowerShell) — `install.ps1`

| Source (`~/dotfiles/`) | Destination |
|---|---|
| `.gitconfig` | `~\.gitconfig` |
| `.vimrc` | `~\.vimrc` |
| `.vimrc_vs` | `~\.vimrc_vs` |
| `.config\powershell\Microsoft.PowerShell_profile.ps1` | `~\Documents\PowerShell\Microsoft.PowerShell_profile.ps1` |
| `.config\Code\User\settings.json` | `~\AppData\Roaming\Code\User\settings.json` |
| `.config\Code\User\keybindings.json` | `~\AppData\Roaming\Code\User\keybindings.json` |
| `.claude\CLAUDE.md` | `~\.claude\CLAUDE.md` |
| `.claude\commands` | `~\.claude\commands` |
| `.claude\hooks` | `~\.claude\hooks` |
| `.claude\statusline-command.ps1` | `~\.claude\statusline-command.ps1` |
| `.codex\hooks` | `~\.codex\hooks` |

### macOS / WSL2 / Linux — `install.sh`

| Source (`~/dotfiles/`) | Destination (`~/`) |
|---|---|
| `.gitconfig` | `~/.gitconfig` |
| `.vimrc` | `~/.vimrc` |
| `.vimrc_vs` | `~/.vimrc_vs` |
| `.zsh` | `~/.zsh` |
| `.zshrc` | `~/.zshrc` |
| `.claude/settings.json` | `~/.claude/settings.json` |
| `.claude/statusline.py` | `~/.claude/statusline.py` |
| `.claude/CLAUDE.md` | `~/.claude/CLAUDE.md` |
| `.claude/commands` | `~/.claude/commands` |
| `.claude/hooks` | `~/.claude/hooks` |
| `.codex/hooks` | `~/.codex/hooks` |
| `.config/Code/User/settings.json` | macOS: `~/Library/Application Support/Code/User/settings.json` / Linux: `~/.config/Code/User/settings.json` |
| `.config/Code/User/keybindings.json` | macOS: `~/Library/Application Support/Code/User/keybindings.json` / Linux: `~/.config/Code/User/keybindings.json` |

## Key Conventions

**Per-machine git identity**: `~/dotfiles/.gitconfig` は `[include] path = ~/.gitconfig.local` だけを持ち、`[user]` セクションは含まない。実際の `name` / `email` はマシンごとに `~/.gitconfig.local`（git-ignored）で設定する。`install.sh` / `install.ps1` が初回実行時に空の `~/.gitconfig.local` を生成するので、そこに identity を書き込んでから commit する。未記入のままだと `git commit` が "Please tell me who you are" で失敗する（fail-loud）。

**Local PowerShell overrides**: `~/dotfiles/.ps1/*.ps1` files are auto-loaded from the PowerShell profile. ディレクトリ全体が git-ignored（`.gitkeep` のみ tracked）なので、マシン固有設定の置き場として使う。

**Local zsh overrides**: `~/dotfiles/.zsh/*.zsh` files are auto-loaded at the end of `.zshrc`. ディレクトリ全体が git-ignored（`.gitkeep` のみ tracked）なので、マシン固有設定の置き場として使う。

**`settings.json` setup**: `install.sh` / `install.ps1` が自動で `.claude/settings.json.sample` → `.claude/settings.json` にコピーする（git-ignored）。手動でコピーしても可。

**Git hooks**: `.git-hooks/pre-commit` がシークレットパターンを検出してコミットをブロックする。`install.sh` / `install.ps1` が `git config core.hooksPath .git-hooks` を設定する。

**VSCode config**: `settings.json` と `keybindings.json` を `.config/Code/User/` 配下に置き、`install.sh` / `install.ps1` が OS ごとの宛先（macOS: `~/Library/Application Support/Code/User/`、Linux/WSL: `~/.config/Code/User/`、Windows: `~\AppData\Roaming\Code\User\`）に symlink する。VSCode 標準の Settings Sync は使わない。シークレット（API key / Gist ID 等）はコミットしないこと（`.git-hooks/pre-commit` で検出ブロックされる）。Vim emulation は `vim.vimrc.path` で `.vimrc_vs` を参照している。

**clasp (Google Apps Script CLI)**: `npm install -g @google/clasp` でインストール（`install.ps1` / `install.sh` が自動実行）。`push` は `--force` を自動付与（PowerShell / zsh ラッパー済み）。

**Prompt**: Starship を使用。zsh: `starship init zsh`、PowerShell: `starship init powershell`。

**Package management**:
- Windows: `winget-packages.json` + `winget import` (`install.ps1` が自動実行)
- macOS / Linux: `Brewfile` + `brew bundle`
- Linux / WSL2: `Aptfile` + `apt-get`

**macOS system defaults**: `macos.sh` が Dock / Finder / キーリピート等の `defaults write` を行う。`install.sh` 末尾で macOS かつ対話 TTY のときのみプロンプトして実行する（CI / 非対話時はスキップ）。

**`settings.local.json`**: `.claude/settings.local.json` holds session/machine-specific permission overrides. It is tracked in git and layered on top of `settings.json`.

**`~/.claude/CLAUDE.md`**: This file is symlinked as the global Claude Code memory file, so edits here affect Claude's behavior across all projects on this machine.

**Statusline**:
- Windows: `.claude/statusline-command.ps1` — credentials from `~/.claude/.credentials.json`, cache in `$env:TEMP\claude-usage-cache.json`
- macOS/Linux: `.claude/statusline.py` reads Claude session JSON from stdin (no external API calls). Outputs a single line: `model │ ctx ██░░ 42% │ 5h ██░░ 30% │ 7d ████░ 75%`. Uses true-color gradient (green→yellow→red) and fine sub-block characters. Rate limits come directly from `rate_limits.five_hour.used_percentage` and `rate_limits.seven_day.used_percentage` in the stdin JSON (available in Claude Code v2.1.80+).

**Netskope CA certs**:
- Windows: `C:\ProgramData\netskope\stagent\data\nscacert_combined.pem` (PowerShell profile が自動検出)
- Linux/WSL2: `/etc/ssl/certs/ca-certificates.crt` (`.zshrc` が自動検出)

## Claude Hooks

`.claude/hooks/` に `.sh`（macOS/Linux）と `.ps1`（Windows）の両方が存在する:

| Hook | Event | Windows (.ps1) | macOS/Linux (.sh) |
|---|---|---|---|
| `stop` | Stop | `tada.wav` 再生 (5s debounce) | macOS: `Glass.aiff`; Linux: terminal bell |
| `notify` | Notification | Toast 通知 + `ding.wav` (15s debounce) | macOS: `osascript` + `say`; Linux: `notify-send` |
| `posttooluse` | PostToolUse | `ding.wav` 再生 (3s debounce) | macOS: `say`; Linux: `spd-say`/`espeak` |
| `shellcheck` | PostToolUse (Edit/Write) | `.sh` → shellcheck; `.ps1` → PSScriptAnalyzer | `.sh`/`.zsh` → shellcheck |
| `failure-log` | PostToolUseFailure | `~\.claude\logs\errors.jsonl` に追記 | `~/.claude/logs/errors.jsonl` に追記 |

`settings.json.sample` の hook コマンドは Windows (`pwsh -NonInteractive -File ...`) 向け。macOS/Linux で使う場合、`install.sh` が sample からコピーした後に `~/.claude/settings.json` (= `dotfiles/.claude/settings.json`、git-ignored) のコマンドを `bash ~/.claude/hooks/<name>.sh` 形式に手動で書き換える必要がある。

**shellcheck hook の副作用**: PostToolUse の `shellcheck` hook が `.sh` / `.zsh` / `.ps1` 編集ごとに走る。Edit/Write が shellcheck / PSScriptAnalyzer 警告で失敗することがあるので、当該ファイルを編集するときは lint クリーンな差分を心がける。失敗ログは `~/.claude/logs/errors.jsonl` に追記される (`failure-log` hook)。

## Known Issues

- **Windows symlinks**: Developer Mode または管理者権限が必要。`install.ps1` 実行時にエラーが出た場合は設定を確認。
- **PowerShell venv activation**: `sva` / `venva` 関数は `. .\.venv\Scripts\Activate.ps1` を実行する。Starship がプロンプトに venv 名を表示する。
- **NVM on Windows**: `fnm` を推奨（`winget install Schniz.fnm`）。プロファイルで自動初期化される。
- **`.gitconfig` credential helper**: macOS uses `gh auth git-credential`. Windows では `wincred` や `gh` が使われる場合がある。
- Starship 未インストール時はプロンプトがデフォルトになる。`winget install Starship.Starship` で解決。
